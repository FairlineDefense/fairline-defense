...
router.post('/verify-phone', async (req, res, next) => {
    // add a user to list
    const addUserToSMSUrl = `https://a.klaviyo.com/api/v2/list/SKvZ83/subscribe?api_key=${process.env.KLAVIYO_PRIVATE_KEY}`;
...
fetch(addUserToSMSUrl, addUserToSMSOptions)
  ...
  //get the user's id
  const profileIdUrl = `https://a.klaviyo.com/api/v2/people/search?email=${req.user.email}&api_key=${process.env.KLAVIYO_PRIVATE_KEY}`
  const profileIdOptions = {method: 'GET', headers: {accept: 'application/json'}};
...
//update a user's profile to have a six digit code
  const url = `https://a.klaviyo.com/api/v1/person/${profileId}?code=${code}&api_key=${process.env.KLAVIYO_PRIVATE_KEY}`;
